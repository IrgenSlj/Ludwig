"""Drawing backend — OCCT HLR → SVG elevation preview (BRIEF §7, P0.5).

CadQuery's SVG exporter performs OCCT Hidden-Line-Removal projection, so this SVG is a real
derived view, not a screenshot. It is the lightweight at-a-glance PREVIEW; the fabrication-grade
conventioned shop drawing (multi-view, dimensioned, title-blocked DXF) lives in
`backends/shopdrawing.py` and is driven by the IR's SEMANTICS rather than fragile HLR.
